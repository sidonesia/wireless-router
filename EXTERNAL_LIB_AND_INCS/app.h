 ...
 */
  ...
  typedef int (*DrawUTF8Func)(Graphics *g, Point p, char *utf8, int nbytes);
...
int 	app_exec(App *app, char *command);
...
int 	app_set_clipboard_text(App *app, char *text);
 ...
 *  Localisation:
 */
void    app_set_string(App *app, char *key, char *value);
char *  app_get_string(App *app, char *key);
...
FILE *	app_open_file(char *filepath, char *mode);
int 	app_close_file(FILE *f);
int 	app_remove_file(char *filepath);
int 	app_rename_file(char *oldpath, char *newpath);

Folder *app_open_folder(char *folderpath);
char *	app_read_folder(Folder *f);
int 	app_close_folder(Folder *f);
int 	app_make_folder(char *folderpath, int mode);
int 	app_remove_folder(char *folderpath);
int 	app_rename_folder(char *oldpath, char *newpath);

int 	app_file_info(char *filepath);
long	app_file_size(char *filepath);
long	app_file_time(char *filepath);
char *	app_current_folder(void);
int 	app_set_current_folder(char *folderpath);
char *	app_form_file_path(char *folderpath, char *filename);
char *	app_get_file_name(char *filepath);
...
Window *app_new_window(App *app, Rect area, char *name, long flags);
...
void	app_set_window_title(Window *w, char *title);
...
void	app_place_window_controls(Window *w);
...
void	app_set_control_text(Control *c, char *text);
 ...
 *  Labels:
 */
Control *app_new_label(Window *w, Rect r, char *text, int align);
Control *app_add_label(Control *c, Rect r, char *text, int align);
 ...
 *  Push-buttons:
 */
Control *app_new_button(Window *w, Rect r, char *text, ControlFunc fn);
Control *app_add_button(Control *c, Rect r, char *text, ControlFunc fn);
 ...
 *  Check-boxes:
 */
Control *app_new_check_box(Window *w, Rect r, char *text, ControlFunc fn);
Control *app_add_check_box(Control *c, Rect r, char *text, ControlFunc fn);
...
Control *app_new_radio_button(Window *w, Rect r, char *text, ControlFunc fn);
Control *app_add_radio_button(Control *c, Rect r, char *text, ControlFunc fn);
...
Menu *    app_new_menu(MenuBar *mb, char *name);
Menu *    app_new_sub_menu(Menu *m, char *name);
void      app_del_menu(Menu *m);

MenuItem *app_new_menu_item(Menu *m, char *name, unsigned long key, MenuAction fn);
 ...
 *  Tab-pane buttons:
 */
Control *app_new_tab_button(Window *w, Rect r, char *text, ControlFunc fn);
Control *app_add_tab_button(Control *c, Rect r, char *text, ControlFunc fn);


/*
 *  Text fields and text editing areas:
 */
Control *app_new_field(Window *w, Rect r, char *text);
Control *app_add_field(Control *c, Rect r, char *text);
void	 app_set_field_allowed_width(Control *c, int width);
void	 app_set_field_allowed_chars(Control *c, char *allowed);
void	 app_set_field_disallowed_chars(Control *c, char *disallowed);

Control *app_new_password_field(Window *w, Rect r, char *text);
Control *app_add_password_field(Control *c, Rect r, char *text);

Control *app_new_text_box(Window *w, Rect r, char *text);
Control *app_add_text_box(Control *c, Rect r, char *text);

void	 app_cut_text(Control *c);
void	 app_copy_text(Control *c);
void	 app_clear_text(Control *c);
void	 app_paste_text(Control *c);
void	 app_insert_text(Control *c, char *text);
...
void	app_error(App *app, char *message);
void	app_ask_ok(App *app, char *title, char *message);
int 	app_ask_ok_cancel(App *app, char *title, char *message);
int 	app_ask_yes_no(App *app, char *title, char *message);
int 	app_ask_yes_no_cancel(App *app, char *title, char *message);
char *	app_ask_string(App *app, char *title, char *question, char *answer);
char *	app_ask_file_open(App *app, char *title, char *message, char *name);
char *	app_ask_file_save(App *app, char *title, char *message, char *name);
...
int     app_text_line_length(Font *f, int pixel_width, char *utf8, int nb);
int     app_text_width(Font *f, int pixel_width, char *utf8, int nbytes);
int     app_text_height(Font *f, int pixel_width, char *utf8, int nbytes);
char *  app_draw_text(Graphics *g, Rect r, int align, char *utf8, int nbytes);
...
Font *  app_new_font(App *app, char *name, int style, int pixel_height);
void    app_del_font(Font *f);

int     app_font_height(Font *f);
int     app_font_width(Font *f, char *utf8, int nbytes);

Font *  app_find_default_font(App *app);
void    app_change_default_font(char *fontname);
...
Image *	app_read_image(char *filename, int required_depth);
...
int     app_write_image(Image *img, char *filename);
...
FILE *	app_open_resource(char *file_name, char *resource, long *length);
int 	app_file_has_resources(char *file_name);
...
int app_unicode_to_utf8(Char ** source_start, const Char * source_end,
			char ** target_start, const char * target_end);

int app_utf8_to_unicode(char ** source_start, const char * source_end,
			Char ** target_start, const Char * target_end);

int 	app_unicode_char_to_utf8(Char ch, char *utf8);
Char	app_utf8_char_to_unicode(char *utf8);
int 	app_utf8_length(char *utf8);
...
int	app_write_latin1 (FILE *f, char *s, long nbytes);
int	app_write_utf8 (FILE *f, char *s, long nbytes);

char *	app_utf8_to_latin1 (char *s, int *bytes);
char *	app_correct_utf8 (char *s, int *bytes);

int 	app_utf8_is_ascii(char *utf8, long nbytes);
int 	app_utf8_is_latin1(char *utf8, long nbytes);
